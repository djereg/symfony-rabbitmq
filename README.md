...
Since the Symfony event system does not support listening to interfaces on top of many events, the `EventDispatcher` does the trick by calling the Symfony event dispatcher under the hood and pass the `MessagePublishEvent`'s name and a listener
...
Create an event lister class and add the `AsEventListener` attribute like in the example below.
...
This attribute behaves exactly like the Symfony event listener attribute, but adds one more tag to the service, what helps to collect the events listening to.

```php
use Djereg\Symfony\RabbitMQ\Events\Attribute\AsEventListener;

#[AsEventListener('user.created')]
...
```php
use Djereg\Symfony\RabbitMQ\Events\Message\EventMessage;
...
```php
use Djereg\Symfony\RabbitMQ\Events\Attribute\AsEventListener;
use Djereg\Symfony\RabbitMQ\Events\Listeners\MessageEventListener;
...
First of all you have to register the clients in the Symfony service container.
...
services:
    users_client:
        class: Djereg\Symfony\RabbitMQ\RPC\Services\Client
        tags:
